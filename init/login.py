...
class GoLogin:
    def __init__(self, syspath):
        self.sysconfig = _get_yaml(syspath)
        self.httpClint = SESS
        self.urlConf = urls
        # self.randCode = ""
    ...
    def readImg(self, code_url):
        ...
        try:
            ...
            if self.sysconfig['is_aotu_code']:
                if self.sysconfig['aotu_code_type'] == 1:
                    return DamatuApi(self.sysconfig["damatu"]["uesr"], self.sysconfig["damatu"]["pwd"], img_path).main()
                elif self.sysconfig['aotu_code_type'] == 2:
                    rc = RClient(self.sysconfig["ruokuai"]["uesr"], self.sysconfig["ruokuai"]["pwd"])
    ...
    def go_login(self):
        ...
        :return:
        """
        if self.sysconfig['is_aotu_code'] and self.sysconfig['aotu_code_type'] == 1:
            balance = DamatuApi(self.sysconfig["damatu"]["uesr"], self.sysconfig["damatu"]["pwd"]).getBalance()
            if int(balance) < 40:
                raise balanceException('余额不足，当前余额为: {}'.format(balance))
        user, passwd = self.sysconfig["set"]["12306count"][0]["uesr"], self.sysconfig["set"]["12306count"][1]["pwd"]
